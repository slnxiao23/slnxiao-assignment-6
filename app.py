...
def generate_plots(N, mu, sigma2, S):
    ...
    X = np.random.rand(N)
    noise = np.random.normal(mu, np.sqrt(sigma2), size=N)
    Y = noise  # Since there's no relationship, Y is just noise
    ...
    plt.xlabel("X")
    plt.ylabel("Y")
    plt.title(f"Sample Linear Regression\nY = {slope:.2f}X + {intercept:.2f}")
    plt.legend()
    ...
    for _ in range(S):
        ...
        Y_sim = np.random.normal(mu, np.sqrt(sigma2), size=N)
